...
void sendRequest(int sockfd, struct sockaddr *serverAddress, RequestMessage *request)
{
    ...
    while (1)
    {
        ...
        sprintf(buffer, "%d %c %d %d %lf %lf %d",
                request->requestId, request->messageType, request->fuelType,
                request->priceOrRadius, request->coordinates[0], request->coordinates[1],
                request->errorFlag);

        if (sendto(sockfd, buffer, strlen(buffer), 0, serverAddress, serverAdressLength) < 0)
        {
            logMessage(LOG_WARN, "[CLIENT] Erro ao enviar dados");
        }
        else if (recvfrom(sockfd, ackBuffer, sizeof(ackBuffer), 0, serverAddress, &serverAdressLength) < 0)
        {
            logMessage(LOG_WARN, "[CLIENT] Erro ao receber dados");
        }
        else
        {
            sscanf(ackBuffer, "%d %c", &ackNakResponse.requestId, &ackNakResponse.messageType);

            if (ackNakResponse.requestId == request->requestId)
            {
                ...
                else if (ackNakResponse.messageType == 'N')
                {
        ...
        logMessage(LOG_INFO, "[MSG id=%d] Enviando novamente a mensagem", request->requestId);
    }
}
...
void getResponse(int sockfd, struct sockaddr *serverAddress, ResponseMessage *response)
{
    ...
    while (recvfrom(sockfd, buffer, sizeof(buffer), 0, serverAddress, &serverAdressLength) < 0)
    {
        logMessage(LOG_WARN, "[CLIENT] Erro ao receber dados");
        logMessage(LOG_INFO, "[CLIENT] Esperando novamente uma mensagem");
    }
